...
It's no longer just one interface, having been factored into two: `IQueryable` and `IQueryProvider`.Let's just walk through these before we get to actually implementing them.
